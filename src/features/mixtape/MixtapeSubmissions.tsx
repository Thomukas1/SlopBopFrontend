...
//
//   open (count < max) → intro + capacity gauge + form (or the thank-you notice)
    ...
    body = (
      <div className="flex flex-col gap-lg">
        ...
        <div className="flex items-center text-xs font-semibold text-accent">
          <span>{status.track_count} / {status.max_tracks} slots filled</span>
        </div>

        <SongSubmissionForm collectionId={mixtapeId} refresh={refresh} />
      </div>
    );
  } else if (status.reason === 'full') {
    body = (
      <div className="flex flex-col items-center gap-xs text-center py-sm">
